 ...
 *
 * Source: https://github.com/trekhleb/javascript-algorithms/blob/master/src/data-structures/heap/MinHeapAdhoc.js
 */
class MinHeapAdhoc {
  heap: number[];
  ...
  constructor(heap: number[] = []) {
    this.heap = [];
    heap.forEach(this.add.bind(this));
  }
...
function findKthLargest(nums: number[], k: number): number {
  const heap = new MinHeapAdhoc(nums.slice(0, k));
